...
const amith = {
  ...
  focus:    ["backend systems", "compilers", "dev tooling"],
  langs:    ["Java", "C#", "Python", "TypeScript", "Rust"],
  runtime:  ["JVM 21", ".NET 10", "Node", "CPython"],
  data:     ["PostgreSQL", "MySQL", "pgvector", "SQLite"],
  infra:    ["Docker", "Terraform", "Linux"],
  shipping: "documind · seat-wave · SLangJava"
};
```
...
Multi-agent RAG platform. LangGraph corrective-RAG pipeline — router → hybrid retrieval + RRF + rerank → relevance grading → web-search fallback → hallucination check. Streams cited answers with a live agent-status timeline.
...
Event ticketing platform with a race-safe seat reservation engine — PostgreSQL `SERIALIZABLE` + `xmin` optimistic concurrency, real-time SignalR seat maps, JWT auth, Docker Compose.
...
Java port of the SlangSharp (ARLang) compiler: ANTLR front end, type checker, tree-walking interpreter, Java-codegen backend. **174 tests** pinned against the original .NET implementation.
...
Realtime collaborative Trello-style board with live multi-user sync over WebSockets.
...
Thread-safe token-bucket rate limiter — a clean concurrency primitive with full test coverage.
...
[![Blog](https://img.shields.io/badge/Blog-2962FF?style=for-the-badge&logo=hashnode&logoColor=white)](https://your-blog.dev)
-->
